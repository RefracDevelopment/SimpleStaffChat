...
public class CommandManager {
    ...
    public void registerAll() {
        if (this.plugin.getConfig().STAFFCHAT_ENABLED) {
            this.commands.add(new StaffChatCommand(this.plugin));
        }

        if (this.plugin.getConfig().DEVCHAT_ENABLED) {
            this.commands.add(new DevChatCommand(this.plugin));
        }

        if (this.plugin.getConfig().ADMINCHAT_ENABLED) {
            this.commands.add(new AdminChatCommand(this.plugin));
        }

        if ((this.plugin.getConfig()).CHAT_TOGGLE_ENABLED) {
