...
public class AdminChatCommand extends Command {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        if (args.length >= 1) {
            String format = (sender instanceof org.bukkit.entity.Player) ? (this.plugin.getSettings()).ADMINCHAT_FORMAT.replace("%server%", (this.plugin.getSettings()).SERVER_NAME).replace("%player%", sender.getName()).replace("%message%", message) : (this.plugin.getSettings()).CONSOLE_ADMINCHAT_FORMAT.replace("%server%", (this.plugin.getSettings()).SERVER_NAME).replace("%player%", sender.getName()).replace("%message%", message);
